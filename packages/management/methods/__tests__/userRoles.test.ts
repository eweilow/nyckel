...
import { getManagementUserRoles } from "../userRoles";
...
describe("getManagementUserRoles", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("should wait for rate limiting correctly", async () => {
    ...
    await getManagementUserRoles("id", config);
  ...
  it("should fail if error", async () => {
    ...
    const promise = getManagementUserRoles("id", config);
  ...
  it("should return correctly", async () => {
    ...
    const data = await getManagementUserRoles("id", config);
    expect(((fetch as any) as jest.Mock).mock.calls).toMatchSnapshot();
